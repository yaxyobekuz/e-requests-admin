...
/**
 * ResolutionBar for MSK orders (resolved = "confirmed").
 *
...
const ResolutionBar = ({ byStatus = [] }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {total > 0 && (
        <p className="text-xs text-gray-400 text-center">Jami: {total} ta buyurtma</p>
      )}
    </div>
  );
};
...
/**
 * MskStats — analytics charts for the MSK buyurtmalar module.
 *
          ...
          <ResponsiveContainer width="100%" height={210}>
            <BarChart data={categoryData} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(v) => [v, "Soni"]}
              />
              <Bar dataKey="count" fill={CHART_COLORS.purple} radius={[0, 6, 6, 0]} name="Soni" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </Card>
