...
/**
 * ResolutionBar for service reports (resolved = "confirmed").
 *
 * @param {{ byStatus: Array }} props
 * @returns {JSX.Element}
 */
      ...
      <div className="space-y-2.5">
        {statusRows.map((row) => (
          <div key={row.label} className="flex items-center gap-2">
            <span className="size-2.5 rounded-full flex-shrink-0" style={{ background: row.color }} />
            <span className="text-xs text-gray-600 w-36 truncate flex-shrink-0">{row.label}</span>
            ...
            <span className="text-xs font-medium text-gray-700 w-7 text-right">{row.count}</span>
          </div>
        ))}
      </div>
      {total > 0 && (
        <p className="text-xs text-gray-400 text-center">Jami: {total} ta ariza</p>
      )}
    </div>
  );
};
...
/**
 * ServicesStats — analytics charts for the Xizmat arizalari module.
 *
...
const ServicesStats = ({ filters }) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      {/* Top services bar */}
      <Card title="Top xizmatlar">
        {serviceData.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-12">Ma'lumot topilmadi</p>
        ) : (
          <ResponsiveContainer width="100%" height={210}>
            <BarChart data={serviceData} layout="vertical" margin={{ top: 4, right: 16, left: 8, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(v) => [v, "Soni"]}
              />
              <Bar dataKey="count" fill={CHART_COLORS.accent} radius={[0, 6, 6, 0]} name="Soni" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </Card>
