...
/**
 * Generates page numbers array with ellipsis for pagination.
...
const UsersPage = () => {
  ...
  const filters = useMemo(
    ...
    [regionId, districtId, neighborhoodId, houseType, isActive, page],
  );
  ...
  // ALL_VALUE sentinel → "" (empty = no filter), real id stays as-is
  const handleRegionChange = (val) => {
    const id = val === ALL_VALUE ? "" : val;
    setRegionId(id);
    setDistrictId("");
    setNeighborhoodId("");
    setPage(1);
  ...
  const hasFilters =
    regionId || districtId || neighborhoodId || houseType || isActive;
 ...
 * @param {object} props
 * @param {object} props.user - User document (populated address)
 * @param {number} props.index - Row number
 */
